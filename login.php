	...
	if($database->rowCount() <= 0){
		...
		if ($_POST['password'] == $pass['password']) {
			if($pass['active'])
			{
				...
				$_SESSION['id'] = $pass['id'];
				$_SESSION['encid'] = "ET17".str_pad($pass['id'], 4, '0', STR_PAD_LEFT);
				alert('<center>User successfully logged in. Redirecting.</center>');
		        echo '<meta http-equiv="refresh" content="1.0;profile.php">';
				$redirecting=true;
			}
			else {
				alert('<center>Your account is not activated. Please check your mail.<br />								Didn\'t receive verification mail? <a href="resend.php" style="color: darkslateblue;">Resend</a></center>', "warning");
			}
		}
		else {
...
if(isset($_SESSION['logged_in']) && $_SESSION['logged_in'] == true && !isset($redirecting)) {
	...
	echo '<meta http-equiv="refresh" content="0.0;profile.php">';
}
else { ?>
