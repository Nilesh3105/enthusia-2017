...
/*
* donut countdown timer directive. Supports Days, Hours, Minutes, Seconds
* takes a string date, or date in milliseconds
* supports timezone localization
* 
* @param date dateString - date to countdown to
* @param type string - denotes which timers to show, ie: mhs == minutes, hours, seconds
*/

//@todo create a callback function to fire when countdown reaches 0
//@todo change day animation time(7 days vs. 1 month) whether more or less than 7 days away

app.directive("bltCountdownTimer", ["$interval", "$timeout", function($interval, $timeout) {
  var timers = {};
  ...
  var updateSpinners = function(scope) {
    ...
    if(scope.secs <= 0) { killSpinner(scope.style, 'secs') }
    else { scope.style.secs = {'-webkit-animation-delay' : '-' + (60-scope.secs) + 's'}; }
    
    if(scope.mins <= 0) { killSpinner(scope.style, 'mins') }
    else { scope.style.mins = {'-webkit-animation-delay' : '-' + (3600-(scope.mins*60)) + 's'}; }
    
    if(scope.hrs <= 0) { killSpinner(scope.style, 'hrs') }
    else { scope.style.hrs = {'-webkit-animation-delay' : '-' + (86400-(scope.hrs*60*60)) + 's'}; }
    
    if(scope.days <= 0) { killSpinner(scope.style, 'days') }
    else { scope.style.days = {'-webkit-animation-delay' :  '-' + (2419200 - parseInt(scope.timeToDate/1000)) + 's'}; }
  };
